...
document.addEventListener('click', (e) => {
  // Исключаем элементы чат-бота из обработки кликов для sidebar
  ...
  if (
    ...
    (!chatBotWindow || !chatBotWindow.contains(e.target))
  ) {
...
window.contactClient = (index, platform) => {
  ...
  const phone = client.phones[0].replace(/[^0-9+]/g, '');
  let url;
  switch (platform) {
    case 'whatsapp':
...
// Объединённый обработчик DOMContentLoaded
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Инициализация клиентов и сортировки
  renderClients();
  initSortable();
